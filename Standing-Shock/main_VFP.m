...
for dt_index = 1:numel(DTvals)
    ...
    for method_index = 1:numel(methods)
        ...
        leftBC_low_rank = maxwellian_low_rank(n_vals(x_min - dx/2), rvals, zvals, u_para(x_min - dx/2), Tae_vals(x_min - dx/2), R_const);
        rightBC_low_rank = maxwellian_low_rank(n_vals(x_max + dx/2), rvals, zvals, u_para(x_max + dx/2), Tae_vals(x_max + dx/2), R_const);
        ...
        for t_index = 2:Nt
            ...
            switch(method)
                case '1' % IMEX111
                    [n_vals, u_para, Ta_vals, Te_vals, u_hat, nu_hat, S_hat, Q_hat, nTe_hat, kappaTx] = fluid_solver_IMEX111(f_vals_low_rank, n_vals, u_para, Ta_vals, Te_vals, dt, dx, dr, dz, rvals, zvals, qa, qe, ma, me, R_const, x_min, x_max);
                    ...
                    parfor spatialIndex = 1:Nx
                        ...
                        [Vr, S, Vz, rank, E] = IMEX111(Vr, S, Vz, u_perp, u_para, dt, dx, tval, rvals, zvals, x_min, x_max, f_vals_low_rank, ma, me, qa, qe, Ar, Az, Br, Bz, Cr, Cz, tolerance, n_vals, Ta_vals, Te_vals, rhoM, JzM, kappaM, spatialIndex, R_const, leftBC_low_rank, rightBC_low_rank, wr, wz, c, w_norm_1_squared, w_norm_v_squared, w_norm_v2_squared);
                ...
                case '2' % IMEX222
                    ...
                    [n_vals1, u_para1, Ta_vals1, Te_vals1, u_hat1, nu_hat1, S_hat1, Q_hat1, nTe_hat1, kappaTx1] = fluid_solver_IMEX111(f_vals_low_rank, n_vals, u_para, Ta_vals, Te_vals, gamma*dt, dx, dr, dz, rvals, zvals, qa, qe, ma, me, R_const, x_min, x_max);
                    
                    n_vals1_QCM = zeros(Nx, 1);
                    u_para1_QCM = zeros(Nx, 1);
                    Ta_vals1_QCM = zeros(Nx, 1);
                    ...
                    parfor spatialIndex = 1:Nx
                        ...
                        % Add QCM here to correct moments - ONLY USE FOR
                        % COLLISION OPERATOR, KEEP n_vals1, u_para1, Ta_vals1 the same
                        % everywhere else
                        [n_vals1_QCM(spatialIndex), u_para1_QCM(spatialIndex), Ta_vals1_QCM(spatialIndex)] = QCM(rhoM1, JzM1, kappaM1, R_const, rvals, zvals);
                    
                        % discretize Fokker-Planck operators
                        nu_aa1 = n_vals1_QCM(spatialIndex)/(Ta_vals1_QCM(spatialIndex)^(3/2));
                        nu_ae1 = sqrt(2*me)*n_vals1_QCM(spatialIndex)/(Te_vals1(spatialIndex)^(3/2));
                        C_aa_r1 = nu_aa1*GetFokkerPlanckFlux(Ar, Br, Cr, u_perp(spatialIndex), Ta_vals1_QCM(spatialIndex), ma, rvals, tval, dr);
                        C_ae_r1 = nu_ae1*GetFokkerPlanckFlux(Ar, Br, Cr, u_perp(spatialIndex), Te_vals1(spatialIndex), ma, rvals, tval, dr);
                        C_aa_z1 = nu_aa1*GetFokkerPlanckFlux(Az, Bz, Cz, u_para1_QCM(spatialIndex), Ta_vals1_QCM(spatialIndex), ma, zvals, tval, dz);
                        C_ae_z1 = nu_ae1*GetFokkerPlanckFlux(Az, Bz, Cz, u_para1_QCM(spatialIndex), Te_vals1(spatialIndex), ma, zvals, tval, dz);
                        Cr1 = C_aa_r1 + C_ae_r1;
                        Cz1 = C_aa_z1 + C_ae_z1;
                    ...
                    [n_vals2, u_para2, Ta_vals2, Te_vals2, u_hat2, nu_hat2, S_hat2, Q_hat2, nTe_hat2, kappaTx2] = fluid_solver_IMEX222_stage2(f_vals_low_rank_temp, n_vals, u_para, Ta_vals, Te_vals, n_vals1, u_para1, Ta_vals1, Te_vals1, nu_hat1, S_hat1, Q_hat1, nTe_hat1, dt, dx, dr, dz, rvals, zvals, qa, qe, ma, me, R_const, x_min, x_max);

                    n_vals2_QCM = zeros(Nx, 1);
                    u_para2_QCM = zeros(Nx, 1);
                    Ta_vals2_QCM = zeros(Nx, 1);
                    ...
                    parfor spatialIndex = 1:Nx
                        ...
                        % Add QCM here to correct moments - ONLY USE FOR
                        % COLLISION OPERATOR, KEEP n_vals2, etc the same
                        % everywhere else
                        [n_vals2_QCM(spatialIndex), u_para2_QCM(spatialIndex), Ta_vals2_QCM(spatialIndex)] = QCM(rhoM2, JzM2, kappaM2, R_const, rvals, zvals);
                        ...
                        nu_aa2 = n_vals2_QCM(spatialIndex)/(Ta_vals2_QCM(spatialIndex)^(3/2));
                        nu_ae2 = sqrt(2*me)*n_vals2_QCM(spatialIndex)/(Te_vals2(spatialIndex)^(3/2));
                        C_aa_r2 = nu_aa2*GetFokkerPlanckFlux(Ar, Br, Cr, u_perp(spatialIndex), Ta_vals2_QCM(spatialIndex), ma, rvals, tval+dt, dr);
                        C_ae_r2 = nu_ae2*GetFokkerPlanckFlux(Ar, Br, Cr, u_perp(spatialIndex), Te_vals2(spatialIndex), ma, rvals, tval+dt, dr);
                        C_aa_z2 = nu_aa2*GetFokkerPlanckFlux(Az, Bz, Cz, u_para2_QCM(spatialIndex), Ta_vals2_QCM(spatialIndex), ma, zvals, tval+dt, dz);
                        C_ae_z2 = nu_ae2*GetFokkerPlanckFlux(Az, Bz, Cz, u_para2_QCM(spatialIndex), Te_vals2(spatialIndex), ma, zvals, tval+dt, dz);
                        Cr2 = C_aa_r2 + C_ae_r2;
                        Cz2 = C_aa_z2 + C_ae_z2;
                        ...
                        [Vr1_dagger, ~, Vz1_dagger, ~, ~] = IMEX111(Vr0, S0, Vz0, u_perp, u_para2, dt, dx, tval, rvals, zvals, x_min, x_max, f_vals_low_rank, ma, me, qa, qe, Ar, Az, Br, Bz, Cr, Cz, tolerance, n_vals2, Ta_vals2, Te_vals2, rhoM2, JzM2, kappaM2, spatialIndex, R_const, leftBC_low_rank, rightBC_low_rank, wr, wz, c, w_norm_1_squared, w_norm_v_squared, w_norm_v2_squared);
                        ...
                        % S-Step
                        S2_hat = sylvester((eye(size(Vr2_hat, 2)) - (gamma*dt*((rvals .* Vr2_hat)')*(Cr2*Vr2_hat))), gamma*dt*((Dz2 - Cz2)*Vz2_hat)'*Vz2_hat, W1_S2); %((rvals .* Vr2_hat)'*W1*Vz2_hat));
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%% FOLLOWING SECTION FOR TEMPORAL ACCURACY TEST ONLY %%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Compute errors
        % error = zeros(Nx, 1);
        % soln_n = zeros(Nx, 1);
...
legend('||f - f_{exact}||_1', '||n - n_{exact}||_1', '||u - u_{exact}||_1', '||T_a - T_{a, exact}||_1', 'Slope 2', 'Location', 'northwest'); 
fontsize(18,"points");
set(gcf,'Units','pixels','Position', [100 100 800 500]);
% saveas(gcf, './Plots/VFP_temporal_error_IMEX222_QCM.fig');
% exportgraphics(gcf,'./Plots/VFP_temporal_error_IMEX222_QCM.pdf','ContentType','vector');
...
ylim([0, 1.2]);
legend('n', 'u_{||}', 'T_e', 'T_a', 'Location', 'east');
...
% Rank plot
figure(4); clf;
plot(tvals, avg_ranks, 'k', 'LineWidth', 1.5); hold on;
...
function [Vr, S, Vz, rank, E] = IMEX111(Vr0, S0, Vz0, ur, uz, dt, dx, tval, rvals, zvals, x_min, x_max, f_vals_low_rank, ma, me, qa, qe, Ar, Az, Br, Bz, Cr, Cz, tolerance, n_vals, Ta_vals, Te_vals, rhoM, JzM, kappaM, spatialIndex, R_const, leftBC, rightBC, wr, wz, c, w_norm_1_squared, w_norm_v_squared, w_norm_v2_squared)
    dr = rvals(2) - rvals(1);
    dz = zvals(2) - zvals(1);
    Nr = numel(rvals);
    Nz = numel(zvals);
    
    [n_vals_QCM, u_para_QCM, Ta_vals_QCM] = QCM(rhoM, JzM, kappaM, R_const, rvals, zvals);

    % discretize Fokker-Planck operators
    nu_aa = n_vals_QCM/(Ta_vals_QCM^(3/2));
    nu_ae = sqrt(2*me)*n_vals_QCM/(Te_vals(spatialIndex)^(3/2));
    C_aa_r1 = nu_aa*GetFokkerPlanckFlux(Ar, Br, Cr, ur(spatialIndex), Ta_vals_QCM, ma, rvals, tval, dr);
    C_ae_r1 = nu_ae*GetFokkerPlanckFlux(Ar, Br, Cr, ur(spatialIndex), Te_vals(spatialIndex), ma, rvals, tval, dr);
    C_aa_z1 = nu_aa*GetFokkerPlanckFlux(Az, Bz, Cz, u_para_QCM, Ta_vals_QCM, ma, zvals, tval, dz);
    C_ae_z1 = nu_ae*GetFokkerPlanckFlux(Az, Bz, Cz, u_para_QCM, Te_vals(spatialIndex), ma, zvals, tval, dz);
    Cr1 = C_aa_r1 + C_ae_r1;
    Cz1 = C_aa_z1 + C_ae_z1;
    ...
    % discretize velocity explicitly
    [Dx_Vr, Dx_S, Dx_Vz] = GetVlasovLowRank(dt, dx, zvals, f_vals_low_rank, leftBC, rightBC, spatialIndex);

    Vr0_star = Vr0;
    Vz0_star = Vz0;

    W0_K1 = [Vr0, Dx_Vr]*blkdiag(S0, -Dx_S)*([Vz0, Dx_Vz]'*Vz0_star);
    W0_L1 = (((rvals .* Vr0_star)' * [Vr0, Dx_Vr])*blkdiag(S0, -Dx_S)*([Vz0, Dx_Vz]'))';
...
function [n1, u_para1, T_a1, T_e1, u_para0_half_nodes, nu_hat1, S_hat1, Q_hat1, nTe_hat1, kappaTx] = fluid_solver_IMEX111(f_vals_low_rank, n0, u_para0, T_a0, T_e0, dt, dx, dv_perp, dv_para, v_perp, v_para, qa, qe, ma, me, R_const, x_min, x_max)
    ...
    [nu_hat1, S_hat1, Q_hat1] = get_fluxes_low_rank(f_vals_low_rank, v_perp, v_para, R_const, x_min, x_max, dx, dv_perp, dv_para);
...
function [n2, u_para2, T_a2, T_e2, u_para1_half_nodes, nu_hat2, S_hat2, Q_hat2, nTe_hat2, kappaTx2] = fluid_solver_IMEX222_stage2(f1_vals_low_rank, n0, u_para0, T_a0, T_e0, n1, u_para1, T_a1, T_e1, nu_hat1, S_hat1, Q_hat1, nTe_hat1, dt, dx, dv_perp, dv_para, v_perp, v_para, qa, qe, ma, me, R_const, x_min, x_max)
    ...
    [nu_hat2, S_hat2, Q_hat2] = get_fluxes_low_rank(f1_vals_low_rank, v_perp, v_para, R_const, x_min, x_max, dx, dv_perp, dv_para);
...
function [nu_hat, S_hat, Q_hat] = get_fluxes_low_rank(f_vals_low_rank, v_perp, v_para, R_const, x_min, x_max, dx, dv_perp, dv_para)
    Nx = size(f_vals_low_rank, 1);
    [n0, u_para0, T0] = get_boundaries(x_min, x_max);
    
    leftBC_low_rank = maxwellian_low_rank(n0(x_min - dx/2), v_perp, v_para, u_para0(x_min - dx/2), T0(x_min - dx/2), R_const);
    rightBC_low_rank = maxwellian_low_rank(n0(x_max + dx/2), v_perp, v_para, u_para0(x_max + dx/2), T0(x_max + dx/2), R_const);
...
function [f_vals_low_rank] = maxwellian_low_rank(n, v_perp, v_para, u_para, T, R_const)
    Nx = numel(n);
    f_vals_low_rank = cell(Nx, 3);
    for i = 1:Nx
        f_vals_low_rank{i, 1} = exp(-(v_perp.^2)/(2*R_const*T(i)));
        f_vals_low_rank{i, 2} = (n(i)/((2*pi*R_const*T(i))^(3/2)));
        f_vals_low_rank{i, 3} = exp(-((v_para-u_para(i)).^2)/(2*R_const*T(i)));
    end
end
...
function [] = PlotF(f_vals_low_rank, Xmat, Rmat, Zmat2, Nz, fig_num)
    ...
    figure(fig_num); clf;
    contour(Xmat, Zmat2, F, 40,'linewidth',1.2); shading interp;
